 ...
 <body class="bg-gray-800 min-h-screen flex items-center justify-center p-4">
   ...
   <div id="fileSelected" class="hidden">
     ...
     <script>
         ...
         fileUploadArea.addEventListener('dragover', e => { e.preventDefault(); fileUploadArea.classList.add('border-blue-500','bg-gray-600'); });
         fileUploadArea.addEventListener('dragleave', e => { e.preventDefault(); fileUploadArea.classList.remove('border-blue-500','bg-gray-600'); });
         fileUploadArea.addEventListener('drop', e => { e.preventDefault(); fileUploadArea.classList.remove('border-blue-500','bg-gray-600'); if(e.dataTransfer.files.length>0) handleFileSelection(e.dataTransfer.files[0]); });
         fileUploadArea.addEventListener('click', e => { if(e.target===fileUploadArea||e.target.closest('#uploadDefault')) fileInput.click(); });
         ...
         function handleFileSelection(file) {
             ...
             if(!allowedTypes.includes(file.type)&&!file.name.match(/\.(mp3|wav|ogg|flac|m4a|aac)$/i)){ showError('Please select a valid audio file'); return; }
             if(file.size>52428800){ showError('File size must be < 50MB'); return; }


             selectedFile=file; fileName.textContent=file.name; fileSize.textContent=formatFileSize(file.size);
         ...
         processBtn.addEventListener('click', async ()=>{
             ...
             processBtn.disabled=clearBtn.disabled=uploadBtn.disabled=true; processingIndicator.classList.remove('hidden'); placeholderText.classList.add('hidden');
             try{
                 const formData=new FormData(); formData.append('audio',selectedFile);
                 const response=await fetch('/speech-to-text',{method:'POST',headers:{'X-CSRF-TOKEN':csrfToken},body:formData});
                 const data=await response.json();
                 if(data.status==='success'){ displayResult(data.text); } else { showError(data.message||'Failed to process audio file'); }
         ...
         function displayResult(text){ resultWindow.innerHTML=`<div class="bg-green-50 border border-green-200 rounded-lg p-3 mb-2 flex items-center gap-1"><svg class="h-4 w-4 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg><span class="text-green-800 font-medium">Transcription Complete</span></div><div class="bg-gray-800 border border-gray-600 rounded-lg p-3"><div class="flex justify-between items-center mb-1"><h3 class="text-gray-200 font-medium text-sm">Transcribed Text:</h3><button onclick="copyToClipboard()" class="text-blue-400 hover:text-blue-300 text-xs flex items-center gap-1">Copy</button></div><div class="text-gray-100 leading-relaxed whitespace-pre-wrap text-sm" id="transcribedText">${text}</div></div>`; }


         function copyToClipboard(){ const text=document.getElementById('transcribedText').textContent; navigator.clipboard.writeText(text).then(()=>{}); }


         function showError(message){ resultWindow.innerHTML=`<div class="flex items-center justify-center h-full"><div class="text-center"><div class="bg-red-50 border border-red-200 rounded-lg p-3 mb-2 flex items-center gap-1 justify-center"><span class="text-red-800 font-medium">Error</span></div><p class="text-red-700 text-sm mt-1">${message}</p></div></div>`; }


         clearBtn.addEventListener('click', ()=>{
             resultWindow.innerHTML=`<div class="flex items-center justify-center h-full text-gray-400" id="placeholderText"><div class="text-center"><svg class="mx-auto h-10 w-10 text-gray-400 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg><p class="text-base font-medium">Upload an audio file to get started</p><p class="text-xs mt-1">Supported formats: MP3, WAV, OGG, FLAC, M4A, AAC</p></div></div>`; clearFileSelection();
         });
